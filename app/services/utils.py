...
def get_poll(poll_id: UUID) -> Optional[Poll]:
    ...
    print(f'Utils, gettign poll:{poll_id}')
...
def get_choice_id_by_label(poll_id: UUID, choice_label:int) -> Optional[UUID]:
    ...
    return get_choice_id_by_label_given(choice_label, poll)
...
def save_vote(poll_id: UUID, vote: Vote) -> None:
    ...
    redis_client.hincrby(
        f"vote_counts:{poll_id}", 
        str(vote.choice_id), 
        1
    )
    return None

def get_choice_id_by_label_given(label:int, poll : Poll) -> Optional[UUID]:
    if poll is None:
        return None
